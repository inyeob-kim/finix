...
사이드바 **「규칙/메타 관리」** 메뉴(경로 `/rules`)에서 서비스별 **YAML 규칙(현재본·작업본·이력)** 을 조회·등록·편집합니다. 로그인이 필요합니다.

| 상태 | 의미 |
|------|------|
| 작업 중 (draft) | 편집·AI 결과. 테스트케이스 **YAML에서 생성** 불가 |
| 적용됨 (current) | 서비스당 1개 현재본. 테스트케이스 생성에 사용 |
| 이력 (history) | 적용/복원 전 스냅샷. 복원 가능 |

**중요:** YAML을 작업본으로만 두고 **적용**하지 않으면 테스트케이스 관리의 「YAML에서 생성」이 실패합니다. 오류 예: `작업본만 있고 적용된 규칙이 없습니다`.

## YAML 규칙 등록 방법 (단계별)

FINIX에서 「YAML 등록」이란, 서비스 코드(예: `PY027`)에 대한 규칙 YAML을 **작업본**으로 저장한 뒤 **적용**하는 것입니다. 화면은 **규칙/메타 관리**(`/rules`)입니다.
...
3. 규칙마다 `testcases` 행 생성 (`rule_history_id` 저장, provenance)
